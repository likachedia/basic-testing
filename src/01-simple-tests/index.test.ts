...
describe('simpleCalculator tests', () => {
  ...
  test('should subtract two numbers', () => {
    ...
    expect(simpleCalculator({a: 100, b: 5, action: Action.Subtract})).toBe(95);
    expect(simpleCalculator({a: 10, b: -2, action: Action.Subtract})).toBe(12);
  });

  test('should multiply two numbers', () => {
    // Write your test here
    expect(simpleCalculator({a: 2, b: 3, action: Action.Multiply})).toBe(6);
    expect(simpleCalculator({a: 20, b:5, action: Action.Multiply})).toBe(100);
    expect(simpleCalculator({a: -2, b: 3, action: Action.Multiply})).toBe(-6);
  });
  ...
  test('should return null for invalid arguments', () => {
    // Write your test here
    expect(simpleCalculator({a: '2', b: '2', action: Action.Exponentiate})).toBeNull();
    expect(simpleCalculator({a: 's', b: '2', action: Action.Add})).toBeNull();
    expect(simpleCalculator({a: '2', b: 'sg', action: Action.Multiply})).toBeNull();
  });
});
